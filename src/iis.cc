...
int update(mle *e, Vec delta, Vec fsharp, int maxm, Mat datat, const PetscInt *ic,
...
int estimate_params_iis(mle *e)
{
  ...
  ierr = VecDuplicate(e->m->params, &tmp);
  CHKERRQ(ierr);
  ierr = VecSet(tmp, 1.0);
  CHKERRQ(ierr);
  ierr = VecDuplicate(e->m->q, &fsharp);
  CHKERRQ(ierr);

  ierr = MatMultTranspose(e->d->data, tmp, fsharp);
  CHKERRQ(ierr);

  ierr = VecMax(fsharp, &i, &f);
  CHKERRQ(ierr);
  maxm = (int)f;
  ...
  ierr = PetscMalloc(maxm * sizeof(double), &a);
  CHKERRQ(ierr);
  ierr = PetscMalloc(e->d->nClasses * sizeof(double), &vv);
  CHKERRQ(ierr);
  ierr = PetscMalloc(e->d->nClasses * sizeof(int), &ic);
  CHKERRQ(ierr);
  ierr = VecDuplicate(e->m->params, &delta);
  CHKERRQ(ierr);
...
int estimate_params_gis(mle *e)
{
  ...
  ierr = VecDuplicate(e->m->params, &delta);
  CHKERRQ(ierr);
  ierr = VecDuplicate(e->m->params, &eq);
  CHKERRQ(ierr);

  // set up

  ierr = VecSet(e->m->params, 0.0);
  CHKERRQ(ierr);
