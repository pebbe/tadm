...
int 
Decomp1d(int n, int size, int rank, int *s, int *e)
{
    int nlocal, deficit;
...
int
measureFile(Datafile &f, int *nClasses, int *nContexts, int *nFeats, 
	    int *nNZeros, double *c)
{
  double v, vv;
  int ii,jj,k,id,ierr;

  ierr = PetscLogEventBegin(MEASURE_EVENT,0,0,0,0);CHKERRQ(ierr);
  ...
  ierr = PetscLogEventEnd(MEASURE_EVENT,0,0,0,0);CHKERRQ(ierr);
...
int 
Dataset::readEvents(char *filename)
{
  int id, nProcs, ierr;
  ...
  if (valid) {
    ...
    ierr = PetscMalloc((nContexts+1)*sizeof(int),&context);CHKERRQ(ierr);
    ...
    if (nProcs == 1) {
      ...
      ierr = PetscMalloc((nClasses+1)*sizeof(int),&ptr);CHKERRQ(ierr);
      ierr = PetscMalloc(nNZeros*sizeof(int),&row);CHKERRQ(ierr);
      ierr = PetscMalloc(nNZeros*sizeof(double),&val);CHKERRQ(ierr);
      ierr = VecCreateSeq(PETSC_COMM_SELF,nClasses,&p_ref);CHKERRQ(ierr);
      if (memd)
	ierr = VecCreateSeq(PETSC_COMM_SELF,nClasses,&p0);CHKERRQ(ierr);
	...
	if (ii > 0) {
      ...
      ierr = VecAssemblyBegin(p_ref);CHKERRQ(ierr);
      if (memd)
	ierr = VecAssemblyBegin(p0);CHKERRQ(ierr);
      ierr = VecAssemblyEnd(p_ref);CHKERRQ(ierr);
      if (memd)
	ierr = VecAssemblyEnd(p0);CHKERRQ(ierr);

      ierr = MatCreateSeqAIJWithArrays(PETSC_COMM_SELF,nClasses,nFeats,
				       ptr,row,val,&data);CHKERRQ(ierr);
...
#if SCALED
      ierr = VecCreateSeq(PETSC_COMM_SELF,nFeats,&v_ref);CHKERRQ(ierr);
      ierr = VecSet(v_ref,0.0);CHKERRQ(ierr);
#endif
	...
	SETERRQ(PETSC_ERR_SUP,"Multiprocessor MEMD estimation not implemented yet");
      ...
      ierr = PetscMalloc((e1-e0+1)*sizeof(int),&dnz);CHKERRQ(ierr);
      ierr = PetscMalloc((e1-e0+1)*sizeof(int),&nnz);CHKERRQ(ierr);
      ...
      ierr = PetscLogEventBegin(SCAN_EVENT,0,0,0,0);CHKERRQ(ierr);
	...
	if (ii > 0) {
      ...
      ierr = PetscLogEventEnd(SCAN_EVENT,0,0,0,0);CHKERRQ(ierr);
	...
	if (ii > 0) {
      ...
      ierr = VecAssemblyBegin(p_ref);CHKERRQ(ierr);
      ierr = MatAssemblyBegin(data,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
      ierr = VecAssemblyEnd(p_ref);CHKERRQ(ierr);
      ierr = MatAssemblyEnd(data,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
...
int 
Dataset::replaceEvents(Mat U, Mat V)
{
  Mat tmp;
  int ierr;
  ...
  ierr = MatGetSize(U,&nClasses,&nFeats);CHKERRQ(ierr);
  nNZeros = nClasses*nFeats;

  // replace e_ref

  ierr = VecDestroy(e_ref);CHKERRQ(ierr);
  ierr = VecCreateSeq(PETSC_COMM_SELF,nFeats,&e_ref);CHKERRQ(ierr);
...
int 
initializeDataset(Dataset &d, PetscTruth transpose)
{
  int ierr, n, ia, *ii;
  PetscScalar zz, *xx;
  PetscTruth valid;
  ...
  if (!d.initialized) {
    ierr = VecDuplicate(d.z,&d.pmarg);CHKERRQ(ierr);
    double eps;
  ...
  if (d.bootstrap) {
	...
	int p = 0;
	ierr = VecGetArray(d.p_ref,&xx);CHKERRQ(ierr);
	for (int i=0;i<d.nClasses;i++) 
	  for (int j=0;j<xx[i];j++) 
	    ii[p++] = i;
	ierr = VecRestoreArray(d.p_ref,&xx);CHKERRQ(ierr);	    
	...
	srand48(time(NULL));
      ...
      ierr = VecSet(d.p_ref,0.0);CHKERRQ(ierr);
	...
	sum_z(d.p_ref,&d);
	VecCopy(d.z,d.pmarg);
	ierr = VecDuplicate(d.pmarg,&d.stuff);CHKERRQ(ierr);
	ierr = VecCopy(d.pmarg,d.stuff);CHKERRQ(ierr);

	ierr = VecDuplicate(d.p_ref,&d.things);CHKERRQ(ierr);
	ierr = VecCopy(d.p_ref,d.things);CHKERRQ(ierr);
	...
	srand48(time(NULL));
      ...
      Vec temp;
      ierr = VecDuplicate(d.pmarg,&temp);CHKERRQ(ierr);

      ierr = VecSet(temp,0.0);CHKERRQ(ierr);

      ierr = VecGetArray(temp,&xx);CHKERRQ(ierr);
	...
	xx[(int)r] += 1.0;
      ...
      ierr = VecRestoreArray(temp,&xx);CHKERRQ(ierr);	    
  ...
  for(int i=0;i<d.nClasses;i++) {
    int ncols,*cols;
    double *vals;
    ierr = MatGetRow(d.data,i,&ncols,&cols,&vals);CHKERRQ(ierr);
  ...
  if (!d.initialized) {
	...
	if (!in) SETERRQ(PETSC_ERR_FILE_OPEN,"Error opening variances file"); 
	...
	SETERRQ(PETSC_ERR_SUP,"MPI L1 smoothing not supported yet");
      ierr = VecCreateSeq(PETSC_COMM_WORLD,d.nFeats*2,&d.sigma);CHKERRQ(ierr);
      tmp = 1.0/tmp;
      VecSet(d.sigma,tmp);
      // d.lambda = tmp;
      d.penalty = L1;
  ...
  if (!d.initialized) {
    ...
    if (transpose) {
      Mat datat;
      ierr = MatTranspose(d.data, &datat);CHKERRQ(ierr);
      ierr = MatDestroy(d.data);CHKERRQ(ierr);
      d.data = datat;
    }
  }
