...
int writeParams(Model *m, int i)
{
  ...
  {
    ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD, name, &v);
    CHKERRQ(ierr);
    ierr = PetscViewerPushFormat(v, PETSC_VIEWER_ASCII_COMMON);
    CHKERRQ(ierr);
  }
  ...
  if (m->d->reduced)
  {
    ...
    ierr = MatGetSize(m->d->right, &n, &d);
    CHKERRQ(ierr);
    ierr = VecCreateSeq(PETSC_COMM_SELF, n, &x);
    CHKERRQ(ierr);
    ierr = MatMult(m->d->right, m->params, x);
    CHKERRQ(ierr);
    ierr = VecView(x, v);
    CHKERRQ(ierr);
    ierr = VecDestroy(x);
