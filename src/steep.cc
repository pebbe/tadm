...
static int TaoSolve_STEEP(TAO_SOLVER tao, void *solver)
{
  ...
  info = TaoGetSolution(tao, &xx);
  CHKERRQ(info);
  info = TaoGetGradient(tao, &gg);
  CHKERRQ(info);

  info = TaoComputeMeritFunctionGradient(tao, xx, &f, gg);
  CHKERRQ(info);
  info = gg->Norm2(&gnorm);
  CHKERRQ(info); /* gnorm = || gg || */

  while (1)
  {

    info = TaoMonitor(tao, iter++, f, gnorm, 0.0, step, &reason);
    CHKERRQ(info);
    if (reason != TAO_CONTINUE_ITERATING)
      break;

    /* Descent direction */
    info = dx->ScaleCopyFrom(-1.0, gg);
    CHKERRQ(info);
...
static int TaoSetUp_STEEP(TAO_SOLVER tao, void *solver)
{
  ...
  info = xx->Clone(&ctx->DX);
  CHKERRQ(info);
  info = xx->Clone(&ctx->WW);
  CHKERRQ(info);

  info = TaoLineSearchSetUp(tao);
  CHKERRQ(info);
...
static int TaoDestroy_STEEP(TAO_SOLVER tao, void *solver)
{
  ...
  if (tao->setupcalled)
  {
    if (ctx->DX)
    {
      info = TaoVecDestroy(ctx->DX);
      CHKERRQ(info);
    }
    if (ctx->WW)
    {
      info = TaoVecDestroy(ctx->WW);
      CHKERRQ(info);
    }
  }
  info = TaoLineSearchDestroy(tao);
  CHKERRQ(info);
...
static int TaoSetOptions_STEEP(TAO_SOLVER tao, void *solver)
{
  ...
  TaoFunctionBegin;
  info = TaoOptionsHead("Gradient descent method for unconstrained optimization");
  CHKERRQ(info);

  info = TaoLineSearchSetFromOptions(tao);
  CHKERRQ(info);
...
int TaoCreate_STEEP(TAO_SOLVER tao)
{
  ...
  info = TaoSetMaximumIterates(tao, 50);
  CHKERRQ(info);
  info = TaoSetTolerances(tao, 1e-16, 1e-16, 0, 0);
  CHKERRQ(info);

  info = TaoCreateMoreThuenteLineSearch(tao, 0, 0);
  CHKERRQ(info);
