...
int
log_likelihood(Vec p, Vec q, double *f, double *h)
{
  PetscScalar ff = 0.0, hh = 0.0, *pp, *qq;
  int n, ierr;

  ierr = PetscLogEventBegin(ENTROPY_EVENT,0,0,0,0);CHKERRQ(ierr);

  ierr = VecGetLocalSize(p,&n);CHKERRQ(ierr);

  ierr = VecGetArray(p,&pp);CHKERRQ(ierr);
  ierr = VecGetArray(q,&qq);CHKERRQ(ierr);
  ...
  ierr = VecRestoreArray(q,&qq);CHKERRQ(ierr);
    
  ierr = PetscLogFlops(n*7.0);CHKERRQ(ierr);
    
  MPI_Allreduce(&ff,f,1,MPI_DOUBLE,MPI_SUM,PETSC_COMM_WORLD);
  MPI_Allreduce(&hh,h,1,MPI_DOUBLE,MPI_SUM,PETSC_COMM_WORLD);

  ierr = PetscLogEventEnd(ENTROPY_EVENT,0,0,0,0);CHKERRQ(ierr);
...
entropy(Vec x) 
{
  ...
  ierr = PetscLogFlops(n*3.0);CHKERRQ(ierr);
...
int 
sum_z (Vec p, Dataset *d) 
{
  int ierr,high,low,nprocs;
  PetscScalar *xp,*zz;

  ierr = PetscLogEventBegin(SUMZ_EVENT,0,0,0,0);CHKERRQ(ierr);
  MPI_Comm_size(PETSC_COMM_WORLD,&nprocs);

  // compute Z's locally
    
  ierr = VecSet(d->z,0.0);CHKERRQ(ierr);
  ...
  ierr = VecRestoreArray(p,&xp);CHKERRQ(ierr);
  ierr = VecRestoreArray(d->z,&zz);CHKERRQ(ierr);
  ...
  ierr = PetscLogEventEnd(SUMZ_EVENT,0,0,0,0);CHKERRQ(ierr);
...
VecExp(Vec x)
{
  ...
  ierr = PetscLogEventBegin(VECEXP_EVENT,0,0,0,0);CHKERRQ(ierr);
  ierr = VecGetLocalSize(x,&n);CHKERRQ(ierr);
  ierr = VecGetArray(x,&xx);CHKERRQ(ierr);
  ...
  ierr = VecRestoreArray(x,&xx);CHKERRQ(ierr);
  ierr = PetscLogFlops((double)n);CHKERRQ(ierr);
  ierr = PetscLogEventEnd(VECEXP_EVENT,0,0,0,0);CHKERRQ(ierr);
...
int 
normalize(Vec x, Dataset *d)
{
  PetscScalar *xx, *zz;
  int low, high, ierr;

  ierr = PetscLogEventBegin(NORMALIZE_EVENT,0,0,0,0);CHKERRQ(ierr);
  ...
  ierr = PetscLogEventEnd(NORMALIZE_EVENT,0,0,0,0);CHKERRQ(ierr);
...
int 
likelihood (Vec x, double *f, Vec g, mle *e)
{
  int ierr, fail = 0;
  PetscScalar pen;
  ...
  ierr = PetscLogEventBegin(LIKELIHOOD_EVENT,0,0,0,0);CHKERRQ(ierr);
  ...
  if (e->d->penalty==L1||e->d->penalty==BRIDGE) {
    ...
    ierr = VecResetArray(e->p1);CHKERRQ(ierr);
    ierr = VecResetArray(e->p2);CHKERRQ(ierr);
  } else {
  ...
  if (e->d->penalty==L1) {
    ...
    ierr = VecPlaceArray(e->g1,gg);CHKERRQ(ierr);
    ierr = VecPlaceArray(e->g2,gg+e->d->nFeats);CHKERRQ(ierr);
    ...
    ierr = VecResetArray(e->g1);CHKERRQ(ierr);
    ierr = VecResetArray(e->g2);CHKERRQ(ierr);
  ...
  ierr = PetscLogEventEnd(LIKELIHOOD_EVENT,0,0,0,0);CHKERRQ(ierr);
  e->fg++;
